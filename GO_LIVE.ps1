...
# ║    4. Runs full pre-flight check → prints ✅/❌ for every safety layer ║
# ║    5. Starts engine in LIVE mode with the risk scale you chose        ║
# ║                                                                        ║
# ║  Stop with Ctrl-C or run .\STOP_BOT.ps1                               ║
# ╚══════════════════════════════════════════════════════════════════════╝
param(
    [double]$Risk = 0.5   # 0.5 = Phase B half-risk, 1.0 = Phase C full
)
...
Write-Host "       launched: $($mt5.FullName)"
Write-Host "       waiting 20 s for MT5 login + EA attach …"
Start-Sleep -Seconds 20

# 4+5. Activate venv + launch with pre-flight
Write-Host "`n[4/5] Activating .venv …" -ForegroundColor Cyan
& "C:\PropBot\.venv\Scripts\Activate.ps1"

Write-Host "`n[5/5] Starting engine in LIVE mode — pre-flight running …" -ForegroundColor Yellow
