...
#   5. Extract the OOS-only slice (Apr 21 -> today) from the fresh engine run
#   6. Per-trade LIVE vs BACKTEST diagnostic (the actual wiring check)
#   7. Print the verdict
#
...
#
# NOTE: This file is intentionally pure ASCII -- some VPS PowerShell sessions
# read .ps1 files as Windows-1252 and barf on UTF-8 box-drawing characters.
...
Tee-LogLine ""
Tee-LogLine ("-" * 100)
Tee-LogLine "  STEP 1/7  --  Snapshot current LIVE state"
Tee-LogLine ("-" * 100)
...
if not sp.exists():
    print(f'  [X]  MISSING -- bot has never written sizer state (or it was just deleted)')
else:
    ...
    if total_n == 0:
        print('  [!]  WARMUP_STATUS: COLD-START (n_seen total = 0). Bot is sizing at base risk.')
    elif total_n < 15:
        print(f'  [!]  WARMUP_STATUS: WARMING (n_seen total = {total_n} / 15). Still at base risk.')
    else:
        print(f'  [OK] WARMUP_STATUS: WARM (n_seen total = {total_n} >= 15). Sizer using Merton formula.')
...
if not bp.exists():
    ...
    if d.get('halted'):
        print('  [!]  HALTED -- DD breaker is currently active. Bot sizes to zero until reset.')
    else:
...
Tee-LogLine ""
Tee-LogLine ("-" * 100)
Tee-LogLine "  STEP 2/7  --  Pull fresh 3-month M1 data from 5%ers MT5"
Tee-LogLine ("-" * 100)
...
Tee-LogLine ""
Tee-LogLine ("-" * 100)
Tee-LogLine "  STEP 3/7  --  Re-run v30 BASELINE backtest on FRESH data (incl. live window)"
Tee-LogLine ("-" * 100)
...
Tee-LogLine ""
Tee-LogLine ("-" * 100)
Tee-LogLine "  STEP 4/7  --  5-run PARITY AUDIT on FRESH data"
Tee-LogLine ("-" * 100)
...
Tee-LogLine ""
Tee-LogLine ("-" * 100)
Tee-LogLine "  STEP 5/7  --  OOS-only slice (Apr 21 -> today) from the fresh engine run"
Tee-LogLine ("-" * 100)
...
if not p.exists():
    print('  [X]  v30_fresh_trades.json not found -- step 3 must have failed')
else:
