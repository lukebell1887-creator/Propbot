#!/usr/bin/env python3
"""
SHF v5.6 — Hurst-Inverse Dynamic HMM Hold Backtest
=====================================================
Same faithful backtest as test_current_live_faithful.py but with
DYNAMIC HMM hold using the Hurst-inverse formula:

    hold = max(5, min(200, int(100 * (1 - H) / 0.5)))

    H=0.3 → hold=140   (MR pair, vol is dangerous, be patient)
    H=0.4 → hold=120
    H=0.5 → hold=100   (random walk boundary, standard)
    H=0.6 → hold=80    (trending, vol bursts are transient)
    H=0.7 → hold=60    (strongly trending, quick re-entry)

The HMM hold updates every time the Hurst exponent is recomputed.
"""
...
def hurst_inverse_hold(h):
    """Hurst-inverse HMM hold: trending pairs get shorter hold."""
    raw = int(100.0 * (1.0 - h) / 0.5)
    return max(5, min(200, raw))


class HMMRegimeDetector:
    """HMM with DYNAMIC Hurst-inverse hold."""
    def __init__(self, lookback=100):
        ...
        self._min_regime_hold = 100  # will be updated dynamically
...
def run_single_pair(df, pdef):
    """Run full live-bot simulation with Hurst-inverse dynamic HMM hold."""
    ...
    for bar in range(n):
        ...
        if bar > MIN_WARMUP_BARS:
            ...
            # DYNAMIC HMM HOLD: update every bar based on current Hurst
            dynamic_hold = hurst_inverse_hold(h)
            hmm.set_hold(dynamic_hold)
            if bar % 10000 == 0:
                hold_history.append(dynamic_hold)
...
def main():
    print("="*110)
    print("SHF v5.6 — HURST-INVERSE DYNAMIC HMM HOLD BACKTEST")
    print(f"shf_core version: {shf_core.__version__}")
    print("="*110)
    print(f"  Formula: hold = max(5, min(200, int(100 * (1 - H) / 0.5)))")
    print(f"  H=0.3->140  H=0.4->120  H=0.5->100  H=0.6->80  H=0.7->60")
    print(f"  Starting balance: ${STARTING_BALANCE:,.0f}")
    print(f"  All features: Dynamic AKAD, Dwell, Rollover, Sentinel, Corr, Ghost Stop")
    ...
    for pair_name, (df, pdef) in pair_data.items():
        ...
        print(f"\n  {pair_name:<20} {r['trades']:>5} trades ({r['trades_per_month']:.0f}/mo)  "
              ...
              f"AvgHMMhold={r['avg_hmm_hold']:.0f}  ({elapsed:.1f}s){ghost_str}")

    # =========================================================================
    # COMPARISON TABLE: Hurst-Inverse vs Best Static HMM
    # =========================================================================
    print(f"\n\n{'='*120}")
    print("COMPARISON: HURST-INVERSE DYNAMIC vs BEST STATIC HMM (from previous test)")
    print(f"{'='*120}")
    print(f"\n  {'Pair':<20} {'':^55} {'':^55}")
    print(f"  {'':20} {'--- HURST-INVERSE DYNAMIC ---':^55} {'--- BEST STATIC HMM ---':^55}")
    print(f"  {'Pair':<20} {'HMMhold':>7} {'Trades':>7} {'WR':>7} {'PF':>7} {'Return':>8} {'MaxDD':>7} "
          f"{'BestHMM':>7} {'Trades':>7} {'WR':>7} {'PF':>7} {'Return':>8} {'MaxDD':>7} {'Winner':>8}")
    print(f"  {'-'*118}")

    for pair_name, r in results.items():
        prev = prev_results.get(pair_name, {})
        p_hmm = prev.get('best_hmm', '?')
        p_trades = prev.get('trades', 0)
        p_wr = prev.get('wr', 0)
        p_pf = prev.get('pf', 0)
        p_ret = prev.get('return_pct', 0)
        p_dd = prev.get('max_dd_pct', 0)

        # Determine winner by PF (primary) then MaxDD (secondary)
        if r['pf'] > 0 and p_pf > 0:
            if r['pf'] > p_pf * 1.05:
                winner = "DYNAMIC"
            elif p_pf > r['pf'] * 1.05:
                winner = "STATIC"
        ...
        print(f"  {pair_name:<20} {r['avg_hmm_hold']:>7.0f} {r['trades']:>7} {r['wr']:>6.1f}% {r['pf']:>7.2f} "
              f"{r['return_pct']:>7.2f}% {r['max_dd_pct']:>6.2f}%  "
              f"{str(p_hmm):>7} {p_trades:>7} {p_wr:>6.1f}% {p_pf:>7.2f} "
              f"{p_ret:>7.2f}% {p_dd:>6.2f}% {winner:>8}")

    # =========================================================================
    # MONTHLY BREAKDOWN
    # =========================================================================
    print(f"\n\n{'='*110}")
    print("MONTHLY BREAKDOWN (Hurst-Inverse Dynamic HMM)")
    print(f"{'='*110}")
    for pair_name, r in results.items():
        monthly = r.get('monthly', {})
        if not monthly: continue
        print(f"\n  {pair_name} (avg HMM hold={r['avg_hmm_hold']:.0f}):")
    ...
    # Save
    out = Path("Results/hurst_inverse_hmm_results.json")
    def conv(o):
        if isinstance(o, (np.integer,)): return int(o)
        if isinstance(o, (np.floating,)): return float(o)
        if isinstance(o, (pd.Timestamp, datetime)): return str(o)
        return str(o)
    save_data = {'method': 'hurst_inverse', 'formula': 'hold = max(5, min(200, int(100*(1-H)/0.5)))',
