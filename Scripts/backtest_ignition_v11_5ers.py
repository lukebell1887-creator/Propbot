#!/usr/bin/env python3
"""
SHF v11 IGNITION — Hawkes × Kalman × GARCH stochastic-process backtest.

Reports by symbol AND by exit reason so we can see if signal-decay exits
(the "perfect-timing" claim) actually outperform price-based exits.

Usage:
    python Scripts/backtest_ignition_v11_5ers.py
    python Scripts/backtest_ignition_v11_5ers.py --months 3 --hawkes-thresh 2.5
"""
...
from src.ignition_engine import IgnitionEngine, IgnitionConfig, FIVEERS_SPECS  # noqa: E402
...
def run(balance: float, months: int, hawkes_thresh: float, kalman_z: float,
         require_confluence: bool, out_dir: Path):
    ...
    cfg = IgnitionConfig(
        hawkes_ratio_threshold=hawkes_thresh,
        kalman_z_threshold=kalman_z,
        require_confluence=require_confluence,
    )
    symbols = [FIVEERS_SPECS[s] for s in SYMBOLS]
    engine = IgnitionEngine(symbols=symbols, cfg=cfg, initial_equity=balance)

    print(f"\n  Engine: v11 IGNITION (Hawkes x Kalman x GARCH x GPD)")
    print(f"  Gates:  lambda-ratio > {hawkes_thresh}, |mu_hat/sqrt(P)| > {kalman_z}, confluence={require_confluence}")

    # Merge-sort M1 bars across symbols
    merged = []
    ...
    with open(out_dir / f"v11_ignition_5ers_{int(balance)}_{months}m.json", "w") as f:
        json.dump(summ, f, indent=2, default=str)
    engine.dump_trades(str(out_dir / f"v11_ignition_5ers_{int(balance)}_{months}m_trades.json"))
...
def _print_summary(s: dict, balance: float, months: int):
    print("=" * 80)
    print(f"  v11 IGNITION | 5%ers MTB | ${balance:,.0f} | {months} months")
    print("=" * 80)
    ...
    if s.get('trades', 0):
        ...
        print(f"  Avg bars held     {s['avg_bars_held']:>12.1f}  (M5 bars; x5 = min held)")
        ...
        for sym, d in sorted(s['by_symbol'].items(), key=lambda x: -x[1]['net']):
            print(f"    {sym:<8} {d['n']:>4} {d['wr']*100:>6.1f}% {d['expR']:>+8.3f} {d['avg_bars']:>6.1f} {d['net']:>+11,.2f}")

        print("\n  By side:")
        print(f"    {'side':<6} {'n':>4} {'WR':>7} {'expR':>8} {'net$':>11}")
        for side, d in s['by_side'].items():
            label = "LONG" if side in ("1", 1) else "SHORT"
            print(f"    {label:<6} {d['n']:>4} {d['wr']*100:>6.1f}% {d['expR']:>+8.3f} {d['net']:>+11,.2f}")
    ...
    print(f"    Trades >= 20      : {'YES' if s.get('trades', 0) >= 20 else 'NO'}")
...
def main():
    ...
    ap.add_argument("--hawkes-thresh", type=float, default=2.0)
    ap.add_argument("--kalman-z", type=float, default=1.5)
    ap.add_argument("--no-confluence", action="store_true",
                     help="trade on EITHER signal alone (not confluence)")
    ap.add_argument("--out", type=Path, default=ROOT / "Results")
    args = ap.parse_args()
    run(args.balance, args.months, args.hawkes_thresh, args.kalman_z,
         not args.no_confluence, args.out)
