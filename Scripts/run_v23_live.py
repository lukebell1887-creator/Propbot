...
If your broker uses DIFFERENT symbol names than the defaults
(DE40.cash, US30.cash, US500.cash, XAUUSD), override with:

    python Scripts\\run_v23_live.py ^
        --broker-names "DE40=DAX40,US30=DJ30,US500=SPX500,XAUUSD=XAUUSD"

Use Scripts\\probe_broker_symbols.py to auto-discover the names
your broker actually serves.
"""
...
from src.live.v23_live import (
    V23Live, V23LiveConfig,
    V23_SPECS, V23_BROKER_NAMES, SymbolSpec,
    V23_BROKER_TICK_SIZE, V23_BROKER_LOT_STEP, V23_BROKER_MIN_LOT,
)
...
def build_specs(broker_names: Dict[str, str]) -> Dict[str, SymbolSpec]:
    ...
    for sym, default_broker in V23_BROKER_NAMES.items():
...
def preflight_symbols(bridge: MT5Bridge, specs: Dict[str, SymbolSpec]) -> None:
    ...
    If ANY symbol returns 0 bars, abort with a clear actionable message so we
    never start the main loop on a broken symbol mapping. Doing this here
    (instead of letting warmup fail mid-run) produces a far cleaner error.
    """
    print()
...
def main() -> int:
    ...
    p.add_argument("--broker-names", default="",
                   help='override broker symbol names, e.g. '
                        '"DE40=DAX40,US30=DJ30,US500=SPX500,XAUUSD=XAUUSD"')
    p.add_argument("--risk", type=float, default=0.00110, help="base risk pct (0.00110 = 0.110 pct)")
    ...
    print(f"  base risk     : {args.risk*100:.3f}%   (cap = {args.cap_mult:.1f}x = {args.risk*args.cap_mult*100:.3f}%)")
    ...
    # This is cheap (4 x ~200ms) and saves a confusing mid-warmup crash.
    ...
    runner = V23Live(bridge=bridge, cfg=cfg, dry_run=not args.live, specs=specs)
