...
def main() -> int:
    ...
    print( "  HARD DAILY    : 4.0% static-DD intraday  -> flatten + halt entries")
    print( "                  (5ers kills @ 5.0%; 1.0pt buffer for slip)")
    print( "  HARD TOTAL    : 8.0% peak-to-trough DD   -> flatten + lock account")
    print( "                  (5ers kills @ 10.0%; 2.0pt buffer for slip)")
    print(f"  SOFT TOTAL    : {args.account_kill*100:.1f}% rolling DD -> close-all + halt (legacy ceiling)")
    print(f"  SOFT DAILY    : {args.daily_breaker*100:.1f}% intraday DD -> halt new entries (positions stay open)")
