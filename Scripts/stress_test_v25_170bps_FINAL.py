#!/usr/bin/env python3
"""
stress_test_v25_170bps_FINAL.py â€” Stress test v25 at AGGRESSIVE+ base risk of 0.170 %
...
So the bot must stop trading for the day if today's P&L drops below âˆ’4 %.
The rolling DD breaker trips at 4 % as a second, slower safety net.

Output:
  Results/stress_test_v25_170bps_FINAL.txt
  Results/stress_test_v25_170bps_FINAL.json
"""
...
# =============================================================================
#  V25 ULTRA CONFIG â€” base_risk = 0.170 % (the top of the safe frontier)
# =============================================================================
V25_170_FINAL_SIZER_CFG = MertonGZSizerConfig(
    base_risk_pct=0.00170,   # <<< +63 % vs v24's 0.110 %, +9 % vs v25's 0.165 %
    cap_mult=5.0,            # per-trade cap still 0.900 % (5Ã— base)
    gamma=3.0,
    ewma_alpha=0.20,
    warmup_trades=15,
    dd_cap_pct=0.04,
...
def apply_daily_halt(trades, starting_balance: float, halt_pct: float):
    ...
    P&L has already breached âˆ’halt_pct Ã— starting_balance is dropped, and
    ...
    by_day        = defaultdict(float)          # calendar-day â†’ cum P&L (realised)
    halted_days   = set()
    out = []
    dropped = 0
...
def run_one_scenario(sc_key: str, *, symbols=None) -> dict:
    ...
    t0 = time.time()
    raw_trades, wmin, wmax = btv22.run_portfolio(symbols, V25_170_FINAL_SIZER_CFG)
...
def fmt_scen_line(r: dict, sc) -> str:
    ...
    vcolor = {"PASS": "âœ…", "WARN": "âš ï¸ ", "FAIL": "âŒ"}[verdict]
...
def main():
    ...
    p("=" * 120)
    p("  V25-ULTRA STRESS TEST â€” AGGRESSIVE+ no-chase config @ 0.170 % base risk")
    p("  Bot: v23 ORB + news rails + Merton-GZ (base=0.170 %, cap=5x, Î³=3.0, DD_cap=4 %)")
    ...
    p("     today's P&L  <= -4 %  â†’  daily halt, no more trades today")
    p("     rolling DD   >= 4 %   â†’  flatten + lock for week")
    p("     5ers daily limit  = 5 %  (1 % safety below)")
    p("     5ers total limit  = 8 %  (4 % safety below)")
    p("=" * 120)
    p("")
    p(f"  Running {len(SCENARIOS)} scenarios Ã— {len(SYMS)} symbols ...")
    p("")

    results = []
    for sc in SCENARIOS:
        print(f"    [{sc.severity:>2}] {sc.key:<16} â€” {sc.label}", flush=True)
        try:
    ...
    p("")
    p("=" * 120)
    p("  HEADLINE TABLE  â€”  base_risk = 0.170 %")
    p("=" * 120)
    p(f"  {'Legend':<14}  ðŸŸ¢ good   âšª baseline   ðŸŸ  stress   ðŸ”´ severe   â˜ ï¸  catastrophe")
    p("")
    for sc, r in results:
        if "error" in r:
            p(f"  {sev_icon(sc.severity)} {sc.label:<36} âŒ ERROR: {r['error']}")
        else:
    ...
    p("")
    p("=" * 120)
    p("  EXIT-GATE ANALYSIS â€” survives the 5ers challenge @ 0.170 %?")
    p("  Rule: DD â‰¤ 4 %  AND  WorstDay â‰¥ -4 %  AND  (ret > 0  OR  catastrophic)")
    p("=" * 120)
    ...
    p(f"\n  âœ… SURVIVED ({len(survives)}/{len(SCENARIOS)}):")
    ...
    p("")
    p("=" * 120)
    p("  SUMMARY  â€”  v25-ULTRA @ 0.170 % base risk")
    p("=" * 120)
    ...
    if bl:
        ...
        p(f"    (v25 @ 0.165 % baseline was $+27,023 / 3.09 % DD â€” "
          f"this is the same bot with +9 % base risk)")
    p("=" * 120)

    # Save
    out_dir = ROOT / "Results"; out_dir.mkdir(exist_ok=True)
    with open(out_dir / "stress_test_v25_170bps_FINAL.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    json_data = {
        "bot": "v25-ULTRA = v23 signal + no-chase(300s) + Merton-GZ(0.170%, cap=5x, Î³=3.0, DD_cap=4%) + 4%-daily-halt",
        "sizer_cfg": asdict(V25_170_FINAL_SIZER_CFG),
    ...
    with open(out_dir / "stress_test_v25_170bps_FINAL.json", "w") as f:
        json.dump(json_data, f, indent=2, default=str)
    p(f"\n  Saved: Results/stress_test_v25_170bps_FINAL.txt + .json")
