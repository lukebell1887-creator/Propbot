...
# ----------------------------------------------------------------------------
Write-Host "Running v30.3 preflight (live-engine contract verifier)..." -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "       Fix the failing checks and re-run .\GO_DRYRUN_V30.ps1" -ForegroundColor Yellow
    exit 3
}
Write-Host ""
